...
class ChatWindow(QWidget):
    ...
    def process_bot_reply(self, user_text):
        # Add user message to message history for context
        self.messages.append(("user", user_text)) # <--------------------------------------------------------- Store user message
        # Stream reply from the graph
        try:
            for event in my_graph.stream({"messages": self.messages}):
                for value in event.values():
                    reply = value["messages"][-1].content
                    self.append_chat("Assistant", reply)
                    self.messages.append(("assistant", reply)) # <--------------------------------------------------------- Store assistant reply
